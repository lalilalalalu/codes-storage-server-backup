...
do 
	...
	do
		let rem=600
		printf "#!/bin/sh \nsrun --ntasks-per-node=$PROCS --nodes=$NODES $BUILD_DIR/src/client/client-mul-wklds --extramem=20310720 --sync=1 --rank-alloc-file=${dist}-alloc-8832-${alloc}_${rem}.conf --workload-conf-file=workload-$alloc-$rem.conf --lp-io-dir=bb-model-adap-$alloc-nearest-$dist --lp-io-use-suffix=1 --codes-config=$CONF_DIR/test-checkpoint-dfly-1T.conf >& dragonfly-bck-nearest-$network_size-$alloc-adap-1T-$dist.out" >& example-script-$alloc-adap-nearest-$dist-bck.sh
		#printf "#!/bin/sh \nsrun --ntasks-per-node=$PROCS --nodes=$NODES $BUILD_DIR/src/client/client-mul-wklds --extramem=20310720 --sync=1 --random-bb=1 --rank-alloc-file=${dist}-alloc-8832-${alloc}_${rem}.conf --workload-conf-file=workload-$alloc-$rem.conf --lp-io-dir=bb-model-adap-$alloc-random-$dist --lp-io-use-suffix=1 --codes-config=$CONF_DIR/test-checkpoint-dfly-1T.conf >& dragonfly-bck-random-$network_size-$alloc-adap-1T-$dist.out" >& example-script-min-$alloc-random-$dist-bck.sh
		#printf "#!/bin/sh \nsrun --ntasks-per-node=$PROCS --nodes=$NODES $BUILD_DIR/src/client/client-mul-wklds --extramem=20310720 --sync=1 --random-bb=1 --rank-alloc-file=${dist}-alloc-8832-${alloc}_${rem}.conf --workload-conf-file=workload-$alloc-$rem.conf --lp-io-dir=bb-model-pa-$alloc-random-$dist --lp-io-use-suffix=1 --codes-config=$CONF_DIR/test-checkpoint-dfly-1T.conf >& dragonfly-bck-random-$network_size-$alloc-pa-1T-$dist.out" >& example-script-pa-$alloc-random-$dist-bck.sh
	done
done
