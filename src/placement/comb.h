...
static uint64_t choose(uint64_t n, uint64_t k){
    uint64_t res = 1, i;
    for (i = 1; i <= k; i++){
        res = (res*(n+1-i)) / i;
    }
...
static uint64_t comb_index(unsigned long k, unsigned long *vals){
 ...
 * calculation */
static void rev_ins_sort(unsigned int n, uint64_t *vals){
    unsigned int i, j;
    for (i = 1; i < n; i++){
        uint64_t val = vals[i];
